...
/** 将字段表达式编译为 SQL */
export function compileField(dialect: Dialect, field: string, op: Op, value: Value | Values | undefined, offset = 0): InternalSQL {
...
/** 将字段表达式编译为完整 SQL 结果 */
export function compileFieldToSQL(dialect: Dialect, field: string, op: Op, value: Value | Values | undefined, offset = 0): SQLResult {
...
export function compileFieldConditionRaw(dialect: Dialect, field: string, condition: OrmFieldCondition): string {
  const f = dialect.quote(field);
  ...
  if (isOrmNotLikeCondition(condition)) return `${f} NOT LIKE ${dialect.escape(`%${condition.$notLike}%`)}`;
...
export function compileWhereRaw(dialect: Dialect, where: OrmWhereCondition): string {
  ...
  for (const [key, value] of Object.entries(where)) {
    ...
    if (key === "$or" && Array.isArray(value)) {
      const orClauses = value.map((c) => compileWhereRaw(dialect, c as OrmWhereCondition)).filter(Boolean);
    ...
    if (key === "$and" && Array.isArray(value)) {
      const andClauses = value.map((c) => compileWhereRaw(dialect, c as OrmWhereCondition)).filter(Boolean);
...
export function compileFieldCondition(dialect: Dialect, field: string, condition: OrmFieldCondition, offset = 0): InternalSQL {
  if (condition === null) {
  ...
  if (isOrmNeCondition(condition)) return compileField(dialect, field, Op.Neq, condition.$ne, offset);
  if (isOrmGtCondition(condition)) return compileField(dialect, field, Op.Gt, condition.$gt, offset);
  if (isOrmGteCondition(condition)) return compileField(dialect, field, Op.Gte, condition.$gte, offset);
  if (isOrmLtCondition(condition)) return compileField(dialect, field, Op.Lt, condition.$lt, offset);
  if (isOrmLteCondition(condition)) return compileField(dialect, field, Op.Lte, condition.$lte, offset);
  if (isOrmLikeCondition(condition)) return compileField(dialect, field, Op.Like, condition.$like, offset);
  if (isOrmNotLikeCondition(condition)) return compileField(dialect, field, Op.NotLike, condition.$notLike, offset);
  if (isOrmInCondition(condition)) return compileField(dialect, field, Op.In, condition.$in, offset);
  if (isOrmNotInCondition(condition)) return compileField(dialect, field, Op.NotIn, condition.$notIn, offset);
  if (isOrmIsNullCondition(condition)) return compileField(dialect, field, Op.IsNull, undefined, offset);
  if (isOrmIsNotNullCondition(condition)) return compileField(dialect, field, Op.NotNull, undefined, offset);
  if (isOrmBetweenCondition(condition)) return compileField(dialect, field, Op.Between, condition.$between, offset);
...
export function compileWhere(dialect: Dialect, where: OrmWhereCondition, offset = 0): InternalSQL {
  ...
  for (const [key, value] of Object.entries(where)) {
    ...
    const [sql, vals] = compileFieldCondition(dialect, key, value as OrmFieldCondition, offset + params.length);
    parts.push(sql);
    params.push(...vals);
  }
...
export function compileWhereToSQL(dialect: Dialect, where: OrmWhereCondition, offset = 0): SQLResult {
