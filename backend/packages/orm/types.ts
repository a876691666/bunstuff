...
/** 数字列构建器接口（支持自增） */
export interface NumberColumnBuilder<Nullable extends boolean> extends ColumnBuilder<
  'number',
  Nullable
> {
  /** 设置为可空 */
...
> = T extends 'number' ? NumberColumnBuilder<Nullable> : ColumnBuilder<T, Nullable>

/** 从列配置推断 TypeScript 类型 */
export type InferColumnType<C extends ColumnBuilder<any, any>> =
  C extends ColumnBuilder<infer T, infer N>
    ? N extends true
