...
export class DictService {
  ...
  /** 获取字典类型列表 */
  async findAllTypes(query?: { page?: number; pageSize?: number; name?: string; type?: string; status?: number }) {
  ...
  /** 获取字典数据列表 */
  async findAllData(query?: { page?: number; pageSize?: number; dictType?: string; label?: string; status?: number }) {
    const page = query?.page ?? 1;
    const pageSize = query?.pageSize ?? 10;
    const offset = (page - 1) * pageSize;
    ...
    const data = await DictData.findMany({ where: w.isEmpty() ? undefined : w, limit: pageSize, offset });
