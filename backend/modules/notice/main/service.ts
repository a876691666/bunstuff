...
export class NoticeService {
  /** 获取通知公告列表 */
  async findAll(query?: { page?: number; pageSize?: number; title?: string; type?: string; status?: number }) {
  ...
  /** 获取用户的通知列表（带已读状态） */
  async findAllForUser(userId: number, query?: { page?: number; pageSize?: number; isRead?: boolean }) {
    const page = query?.page ?? 1;
    const pageSize = query?.pageSize ?? 10;
    const offset = (page - 1) * pageSize;
