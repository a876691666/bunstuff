...
export class SeedService {
  ...
  /** 执行所有未执行的 Seeds */
  async runAll(force = false): Promise<{ total: number; success: number; failed: number; results: any[] }> {
    const results: any[] = [];
    let success = 0;
    let failed = 0;
  ...
  async autoRun(): Promise<void> {
    ...
    console.log(`[Seed] 执行完成: 总计 ${result.total}, 成功 ${result.success}, 失败 ${result.failed}`);
