...
export const vipAdminController = new Elysia({ prefix: "/vip", tags: ["管理 - VIP"] })
  ...
  .get("/tier", async ({ query }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:tier:list"] },
    },
  })

  /** 根据 ID 获取 VIP 等级 */
  .get("/tier/:id", async ({ params }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:tier:read"] },
    },
  })

  /** 创建 VIP 等级 */
  .post("/tier", async ({ body }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:tier:create"] },
    },
  })

  /** 更新 VIP 等级 */
  .put("/tier/:id", async ({ params, body }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:tier:update"] },
    },
  })

  /** 删除 VIP 等级 */
  .delete("/tier/:id", async ({ params }) => {
    const existing = await vipService.findTierById(params.id);
    if (!existing) return R.notFound("VIP 等级");
    try {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:tier:delete"] },
    },
  })
  ...
  /** 获取 VIP 等级的资源限制 */
  .get("/tier/:id/resource-limits", async ({ params }) => {
    ...
    detail: {
      ...
      description: "获取指定 VIP 等级的所有资源限制\n\n🔐 **所需权限**: `vip:resource-limit:list`",
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource-limit:list"] },
    },
  })

  /** 创建资源限制 */
  .post("/resource-limit", async ({ body }) => {
    try {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource-limit:create"] },
    },
  })

  /** 更新资源限制 */
  .put("/resource-limit/:id", async ({ params, body }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource-limit:update"] },
    },
  })

  /** 删除资源限制 */
  .delete("/resource-limit/:id", async ({ params }) => {
    const existing = await vipService.findResourceLimitById(params.id);
    if (!existing) return R.notFound("资源限制");
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource-limit:delete"] },
    },
  })
  ...
  .get("/user/:userId", async ({ params }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:user:read"] },
    },
  })
  ...
  .post("/upgrade", async ({ body }) => {
    ...
    detail: {
      ...
      description: "升级用户 VIP 等级，创建待确认的绑定记录，需要调用确认接口完成绑定\n\n🔐 **所需权限**: `vip:user:upgrade`",
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:user:upgrade"] },
    },
  })

  /** 直接升级用户 VIP */
  .post("/upgrade-direct", async ({ body }) => {
    try {
    ...
    detail: {
      summary: "直接升级用户 VIP",
      description: "直接升级用户 VIP 等级，立即生效，无需确认\n\n🔐 **所需权限**: `vip:user:upgrade`",
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:user:upgrade"] },
    },
  })
  ...
  /** 确认 VIP 绑定 */
  .post("/confirm-binding", async ({ body }) => {
    try {
    ...
    detail: {
      summary: "确认 VIP 绑定",
      description: "确认或取消 VIP 绑定，确认后将更新用户角色\n\n🔐 **所需权限**: `vip:user:confirm`",
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:user:confirm"] },
    },
  })

  /** 取消用户 VIP */
  .post("/cancel/:userId", async ({ params }) => {
    try {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:user:cancel"] },
    },
  })
  ...
  /** 检查资源使用 */
  .post("/resource/check", async ({ body }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource:check"] },
    },
  })

  /** 增加资源使用 */
  .post("/resource/increment", async ({ body }) => {
    try {
      const data = await vipService.incrementResourceUsage(body.userId, body.resourceKey, body.amount);
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource:increment"] },
    },
  })

  /** 获取用户资源使用情况 */
  .get("/resource/usage/:userId", async ({ params }) => {
    ...
    detail: {
      ...
      security: [{ bearerAuth: [] }],
      scope: { permissions: ["vip:resource:usage"] },
    },
  });
