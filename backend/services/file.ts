...
/** 设置配置 */
export function setConfig(cfg: Partial<FileServiceConfig>) {
...
export async function findAll(query?: PageQuery, ctx?: CrudContext) {
  ...
  const where = buildWhere(SysFile.tableName, query?.filter, ctx)
...
export async function findById(id: number, ctx?: CrudContext) {
  return SysFile.findOne({ where: buildWhere(SysFile.tableName, `id = ${id}`, ctx) })
}

/** 创建文件记录 */
export async function create(data: Insert<typeof SysFile>, ctx?: CrudContext) {
  if (!checkCreateScope(SysFile.tableName, data as Record<string, any>, ctx)) return null
  return SysFile.create(data)
}
...
export async function uploadLocal(
  ...
  uploadBy: number,
  ctx?: CrudContext,
): Promise<Row<typeof SysFile> | null> {
  const buffer = await file.arrayBuffer()
  const md5 = Bun.hash(buffer).toString(16)

  // 检查是否已存在相同文件
  const existing = await findByMd5(md5)
...
export async function uploadS3(
  ...
  const { bucket, endpoint } = config.s3Config
...
/** 删除文件 */
export async function remove(id: number, ctx?: CrudContext): Promise<boolean> {
  const file = await findById(id, ctx)
  ...
  } else if (file.storageType === 's3' && config.s3Config) {
    const { bucket, endpoint } = config.s3Config
    const url = `${endpoint}/${bucket}/${file.storagePath}`
    await fetch(url, { method: 'DELETE' })
  }

  const w = buildWhere(SysFile.tableName, `id = ${id}`, ctx)
  if (!w) return false
  return (await SysFile.deleteMany(w)) > 0
}
...
/** 获取文件URL */
export function getFileUrl(file: Row<typeof SysFile>, baseUrl: string = ''): string {
  if (file.storageType === 'local') {
