...
export async function create(data: Insert<typeof MyTable>, ctx?: CrudContext) {
  if (!checkCreateScope(MyTable.tableName, data as Record<string, any>, ctx)) return null
...
```typescript
function buildWhere(tableName: string, filter?: string, ctx?: CrudContext): string | undefined
```
...
```typescript
function checkCreateScope(tableName: string, data: Record<string, any>, ctx?: CrudContext): boolean
```
...
export async function create(data: Insert<typeof T>, ctx?: CrudContext) {
  if (!checkCreateScope(T.tableName, data as Record<string, any>, ctx)) return null
