...
async function handleLogin() {
  ...
  loading.value = true
  try {
    const result = await apiLogin(form.value.username, form.value.password)
    setAuth(result)
    router.push('/space')
  } catch (err: unknown) {
    ...
    <div class="w-120 shrink-0 flex items-center justify-center p-10 relative">
      ...
      <div class="w-full max-w-85 flex flex-col gap-6">
        <div class="flex flex-col gap-1">
          ...
          <n-text depth="3" class="text-sm">登录您的账号</n-text>
        </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="flex flex-col gap-1.5">
            <n-text class="text-sm font-medium">用户名</n-text>
            <n-input
          ...
          <div class="flex flex-col gap-1.5">
            <div class="flex items-center justify-between">
              <n-text class="text-sm font-medium">密码</n-text>
